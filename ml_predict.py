...
if __name__ == '__main__':
    ...
    validation_images_path = os.path.sep.join([config.ORIG_INPUT_DATASET, config.VAL])
    ...
    f = open("/Volumes/MacBackup/Food-5k/vgg16/output/model.cpickle", 'rb')
    ...
    for i, feature in enumerate(image_features):
        ...
        if pred_label != labels[i]:
            ...
            print(f"Image: {imagePaths[i]} Actual: {labels[i]}  Predicted: {pred_label}")

    print(f"Model: {model_name}")
    print(f"Accuracy: {(len(imagePaths) - len(image_errors)) / len(imagePaths):.2f}")
    print(f"Total Images: {len(imagePaths)}")
    print(f"Total Errors: {len(image_errors)}")
